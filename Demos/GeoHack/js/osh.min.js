...
OSH.Exception.Exception=function(msg,errorObj){var error=Error.apply(this,arguments);return error.name=this.name="OSH Exception",this.message=error.message,this.originalErrorObject=errorObj,this.stack=error.stack,this},OSH.Exception.Exception.prototype=Object.create(Error.prototype,{constructor:{value:OSH.Exception.Exception}}),OSH.Exception.Exception.prototype.printStackTrace=function(){return this.stack},OSH.Exception.Exception.prototype.getMessage=function(){return this.message},OSH.Exception.Exception.prototype.getOriginalError=function(){return this.originalErrorObject};
...
OSH.UI.Panel.DialogPanel=OSH.UI.Panel.extend({initialize:function(parentElementDivId,options){this._super(parentElementDivId,options),this.dialogId="dialog-"+OSH.Utils.randomUUID(),this.pinDivId="dialog-pin-"+OSH.Utils.randomUUID();var closeDivId="dialog-close-"+OSH.Utils.randomUUID();this.connectDivId="dialog-connect-"+OSH.Utils.randomUUID(),this.minimizeId="dialog-min-"+OSH.Utils.randomUUID(),this.titleId="dialog-title-"+OSH.Utils.randomUUID(),this.id=this.dialogId,this.name="Untitled";var headerVar="";headerVar+='<div class="header">',this.pinContainerId=null,this.closeable=!1,this.connected=!1,this.swapped=!1,this.connectionIds=[],this.draggable=!1,this.destroyOnClose=!1,this.modal=!1,isUndefined(options)||isUndefined(options.name)||(this.name=options.name),headerVar+='<span class=" line-left" id="'+this.titleId+'">'+this.name+"</span>",headerVar+='   <table class="line-right">',headerVar+="      <tr>",isUndefined(options)||(isUndefined(options.swapId)||""===options.swapId||(this.swapDivId="dialog-exchange-"+OSH.Utils.randomUUID(),headerVar+='<td><i id="'+this.swapDivId+'" class="fa fa-fw pop-icon pop-icon-swap" aria-hidden="true"></i></td>',this.divIdToSwap=options.swapId),!isUndefined(options.connectionIds)&&!isUndefined(options.connectionIds)&&options.connectionIds.length>0&&(headerVar+='<td><i id="'+this.connectDivId+'" class="fa  fa-fw pop-icon pop-icon-connect" aria-hidden="true"></i></td>',this.connected=!0,this.connectionIds=options.connectionIds),isUndefined(options.pinContainerId)||""===options.pinContainerId||(headerVar+='<td><i id="'+this.pinDivId+'" class="fa fa-fw pop-icon pop-icon-unpin" aria-hidden="true"></i></td>',this.pinContainerId=options.pinContainerId),headerVar+='<td><i id="'+this.minimizeId+'" class="fa fa-fw pop-icon pop-icon-min" aria-hidden="true"></i></td>',!isUndefined(options.closeable)&&options.closeable&&(headerVar+='<td><i id="'+closeDivId+'" class="fa fa-fw pop-icon pop-icon-close" aria-hidden="true"></i></td>',this.closeable=options.closeable),!isUndefined(options.draggable)&&options.draggable&&(this.draggable=options.draggable),isUndefined(options.destroyOnClose)||(this.destroyOnClose=options.destroyOnClose),isUndefined(options.modal)||(this.modal=options.modal)),this.modal&&(this.closeable=!0),headerVar+="      </tr>",headerVar+="   </table>",headerVar+="</div>",headerVar+='<div style="clear: both;"></div>',this.parentElementDivId=""===parentElementDivId?document.body:parentElementDivId,this.rootTag=document.getElementById(this.divId),this.rootTag.innerHTML=headerVar,this.rootTag.setAttribute("class","pop-over resizable"),this.rootTag.setAttribute("draggable",this.draggable),this.draggable&&OSH.Utils.addCss(this.rootTag,"draggable");var p=this.rootTag.parentNode;this.outer=document.createElement("div"),this.outer.setAttribute("class",this.modal?"osh dialog modal ":"osh dialog "),this.outer.setAttribute("id",this.id),this.outer.appendChild(this.rootTag),p.appendChild(this.outer),this.keepRatio=!1;var css=this.rootTag.className,keepRatioCss="";isUndefined(options)||(options.css&&(css+=" "+options.css),!isUndefined(options.keepRatio)&&options.keepRatio&&(this.keepRatio=!0,keepRatioCss=" keep-ratio-w")),css+=keepRatioCss,this.rootTag.setAttribute("class",css),this.flexDiv=document.createElement("div"),this.flexDiv.setAttribute("class","pop-inner"),this.popContentDiv=document.createElement("div"),this.popContentDiv.setAttribute("class","pop-content"),this.popContentDiv.setAttribute("id","pop-content-id-"+OSH.Utils.randomUUID()),this.keepRatio||OSH.Utils.addCss(this.popContentDiv,"no-keep-ratio"),this.flexDiv.appendChild(this.popContentDiv),this.rootTag.appendChild(this.flexDiv),this.footer=document.createElement("div"),this.footer.setAttribute("class","footer"),this.footerContent=document.createElement("div"),this.footerContent.setAttribute("id","footer-id-"+OSH.Utils.randomUUID()),this.footer.appendChild(this.footerContent),this.rootTag.appendChild(this.footer),isUndefined(options)||(isUndefined(options.show)||options.show?this.initialWidth=this.rootTag.offsetWidth:(OSH.Utils.addCss(this.outer,"closed"),this.rootTag.style.display="block")),this.rootTag.addEventListener("dragstart",this.drag_start.bind(this),!1),document.addEventListener("dragover",this.drag_over.bind(this),!1),document.addEventListener("drop",this.drop.bind(this),!1),this.closeable&&(document.getElementById(closeDivId).onclick=this.close.bind(this)),null!==this.pinContainerId&&(document.getElementById(this.pinDivId).onclick=this.pin.bind(this)),this.connectionIds.length>0&&(document.getElementById(this.connectDivId).onclick=this.connect.bind(this)),isUndefined(this.swapDivId)||(document.getElementById(this.swapDivId).onclick=this.swapClick.bind(this)),document.getElementById(this.minimizeId).onclick=this.minimize.bind(this);var self=this;OSH.EventManager.observe(OSH.EventManager.EVENT.CONNECT_DATASOURCE,function(event){var dataSources=event.dataSourcesId;if(dataSources.length===self.connectionIds.length&&dataSources.filter(function(n){return self.connectionIds.indexOf(n)!==-1}).length===self.connectionIds.length){var eltDiv=document.getElementById(self.connectDivId);OSH.Utils.addCss(eltDiv,"pop-icon-connect"),OSH.Utils.removeCss(eltDiv,"pop-icon-disconnect"),self.connected=!0}}),OSH.EventManager.observe(OSH.EventManager.EVENT.DISCONNECT_DATASOURCE,function(event){var dataSources=event.dataSourcesId;if(dataSources.length===self.connectionIds.length&&dataSources.filter(function(n){return self.connectionIds.indexOf(n)!==-1}).length===self.connectionIds.length){var eltDiv=document.getElementById(self.connectDivId);OSH.Utils.removeCss(eltDiv,"pop-icon-connect"),OSH.Utils.addCss(eltDiv,"pop-icon-disconnect"),self.connected=!1}}),OSH.EventManager.observe("swap-restore",function(event){self.swapped&&event.exclude!==self.id&&(self.swap(),self.swapped=!1)}),OSH.EventManager.observe(OSH.EventManager.EVENT.SHOW_VIEW,function(event){this.show(event)}.bind(this))},swapClick:function(){OSH.EventManager.fire("swap-restore",{exclude:this.id}),this.swap()},minimize:function(){this.flexDiv.className.indexOf("hide")>-1?(OSH.Utils.removeCss(this.flexDiv,"hide"),OSH.Utils.removeCss(document.getElementById(this.minimizeId),"pop-icon-max"),OSH.Utils.addCss(this.rootTag,"resizable"),OSH.Utils.removeCss(this.rootTag,"minimized"),OSH.Utils.addCss(document.getElementById(this.minimizeId),"pop-icon-min")):(this.flexDiv.setAttribute("class",this.flexDiv.className+" hide"),OSH.Utils.removeCss(document.getElementById(this.minimizeId),"pop-icon-min"),OSH.Utils.addCss(this.rootTag,"minimized"),OSH.Utils.removeCss(this.rootTag,"resizable"),OSH.Utils.addCss(document.getElementById(this.minimizeId),"pop-icon-max"))},swap:function(){var containerDivToSwap=document.getElementById(this.divIdToSwap);if(!isUndefinedOrNull(containerDivToSwap)){if(this.swapped){var popContent=this.popContentDiv.firstChild,swapContainerContent=document.getElementById(this.contentViewId);containerDivToSwap.removeChild(swapContainerContent),this.popContentDiv.removeChild(popContent),containerDivToSwap.appendChild(popContent),this.popContentDiv.appendChild(swapContainerContent),document.getElementById(this.titleId).innerText=this.name,this.swapped=!1,this.keepRatio&&(OSH.Utils.addCss(this.rootTag,"keep-ratio-w"),OSH.Utils.removeCss(this.popContentDiv,"no-keep-ratio"),OSH.Utils.removeCss(containerDivToSwap,"keep-ratio-h"))}else{var popContent=this.popContentDiv.firstChild;this.contentViewId=popContent.id;var swapContainerContent=containerDivToSwap.firstChild;containerDivToSwap.removeChild(swapContainerContent),this.popContentDiv.removeChild(popContent),containerDivToSwap.appendChild(popContent),this.popContentDiv.appendChild(swapContainerContent),this.swapped=!0,document.getElementById(this.titleId).innerText="- Swapped -",this.keepRatio&&(OSH.Utils.removeCss(this.rootTag,"keep-ratio-w"),OSH.Utils.addCss(this.popContentDiv,"no-keep-ratio"),OSH.Utils.addCss(containerDivToSwap,"keep-ratio-h"))}for(var everyChild=document.getElementById(this.divIdToSwap).querySelectorAll("div"),i=0;i<everyChild.length;i++){var id=everyChild[i].id;id.startsWith("view-")&&OSH.EventManager.fire(OSH.EventManager.EVENT.RESIZE+"-"+id)}everyChild=this.popContentDiv.querySelectorAll("div");for(var i=0;i<everyChild.length;i++){var id=everyChild[i].id;id.startsWith("view-")&&OSH.EventManager.fire(OSH.EventManager.EVENT.RESIZE+"-"+id)}}},show:function(properties){(!isUndefinedOrNull(this.contentId)&&properties.viewId.indexOf(this.contentId)>-1||this.id===properties.viewId)&&(OSH.Utils.removeCss(this.outer,"closed"),isUndefined(this.initialWidth)||(this.initialWidth=this.rootTag.offsetWidth))},connect:function(){this.swapped||(this.connected?OSH.EventManager.fire(OSH.EventManager.EVENT.DISCONNECT_DATASOURCE,{dataSourcesId:this.connectionIds}):OSH.EventManager.fire(OSH.EventManager.EVENT.CONNECT_DATASOURCE,{dataSourcesId:this.connectionIds}))},pin:function(){var pinElt=document.getElementById(this.pinDivId),containerElt=document.getElementById(this.parentElementDivId);if(pinElt.className.indexOf("pop-icon-unpin")<=-1){var bodyRect=document.body.getBoundingClientRect(),elemRect=this.rootTag.getBoundingClientRect(),offsetTop=elemRect.top-bodyRect.top,offsetLeft=elemRect.left-bodyRect.left;this.rootTag.setAttribute("draggable",!0),document.getElementById(this.pinContainerId).removeChild(this.rootTag.parentNode),containerElt.appendChild(this.rootTag.parentNode),this.rootTag.style.top=offsetTop,this.rootTag.style.left=offsetLeft,this.rootTag.style.position="absolute",this.draggable=!0,OSH.Utils.removeCss(pinElt,"pop-icon-pin"),OSH.Utils.addCss(pinElt,"pop-icon-unpin")}else this.rootTag.style.top=0,this.rootTag.style.left=0-(this.rootTag.offsetWidth-this.initialWidth),this.rootTag.style.position="relative",this.rootTag.setAttribute("draggable",!1),containerElt.removeChild(this.rootTag.parentNode),document.getElementById(this.pinContainerId).appendChild(this.rootTag.parentNode),this.draggable=!1,OSH.Utils.removeCss(pinElt,"pop-icon-unpin"),OSH.Utils.addCss(pinElt,"pop-icon-pin")},onClose:function(){},close:function(){this.destroyOnClose?this.outer.parentNode.removeChild(this.outer):this.outer.setAttribute("class",this.outer.className+" closed"),this.onClose()},drag_start:function(event){if(event.stopPropagation(),event.target instanceof Element){var style=window.getComputedStyle(event.target,null);event.dataTransfer.effectAllowed="all",event.dataTransfer.setData("text-"+this.rootTag.id,parseInt(style.getPropertyValue("left"),10)-event.clientX+","+(parseInt(style.getPropertyValue("top"),10)-event.clientY))}},drag_over:function(event){return event.stopPropagation(),event.preventDefault(),!1},drop:function(event){event.stopPropagation();var offset=event.dataTransfer.getData("text-"+this.rootTag.id).split(",");return this.rootTag.style.left=100*(event.clientX+parseInt(offset[0],10))/window.innerWidth+"%",this.rootTag.style.top=event.clientY+parseInt(offset[1],10)+"px",event.preventDefault(),!1}});
OSH.UI.Panel.MultiDialogPanel=OSH.UI.Panel.DialogPanel.extend({initialize:function(parentElementDivId,options){this._super(parentElementDivId,options),this.popExtraDiv=document.createElement("div"),this.popExtraDiv.setAttribute("class","pop-extra"),this.popExtraDiv.setAttribute("id","pop-extra-id-"+OSH.Utils.randomUUID()),this.flexDiv.appendChild(this.popExtraDiv)},appendView:function(parentElement,properties){var divToAdd=document.getElementById(parentElement);"none"===divToAdd.style.display&&(divToAdd.style.display="block");var extraDiv=document.createElement("div");extraDiv.setAttribute("class","pop-extra-el");var i=document.createElement("i");i.setAttribute("class","fa fa-caret-right pop-extra-collapse fa-2x"),i.onclick=function(){i.className.indexOf("fa-caret-down")==-1?i.className="fa fa-caret-down pop-extra-show fa-2x":i.className="fa fa-caret-right pop-extra-collapse fa-2x"},extraDiv.appendChild(i),extraDiv.appendChild(divToAdd),this.popExtraDiv.appendChild(extraDiv)},swap:function(){var currentSwapValue=this.swapped;this._super(),this.popExtraDiv.style.display=currentSwapValue?"block":"none"},show:function(properties){this._super(properties)}});
OSH.UI.Panel.SaveDialogPanel=OSH.UI.Panel.DialogPanel.extend({initialize:function(parentElementDivId,properties){this._super(parentElementDivId,properties),this.properties=properties;var footerElt=document.createElement("div");this.footerContent.appendChild(footerElt),this.saveButtonId=OSH.Utils.randomUUID();var strVar="";strVar+='<div class="horizontal-line"></div>',strVar+='<div class="button-edit">',strVar+='  <button id="'+this.saveButtonId+'" class="submit save">Save</button>',strVar+="</div>",footerElt.innerHTML=strVar,OSH.EventManager.observeDiv(this.saveButtonId,"click",this.onSaveClickButtonHandler.bind(this)),OSH.Utils.addCss(this.elementDiv,"save-dialog")},onSaveClickButtonHandler:function(event){this.onSave()},onSave:function(){}});
...
OSH.UI.Panel.VideoPanel=OSH.UI.Panel.StylerPanel.extend({initialize:function(parentElementDivId,options){this._super(parentElementDivId,options)},initPanel:function(){this.contentElt=document.createElement("div"),this.elementDiv.appendChild(this.contentElt),OSH.Helper.HtmlHelper.addHTMLTitledLine(this.contentElt,"Mapping");var hasUIProps=OSH.Utils.hasOwnNestedProperty(this.styler,"properties.ui.video");!hasUIProps&&OSH.Utils.hasOwnNestedProperty(this.styler,"properties.frameFunc")||(hasUIProps?this.initMappingFunctionUI(this.styler.properties.ui.video):this.initMappingFunctionUI())},initMappingFunctionUI:function(properties){this.properties={frame:{datasourceIdx:null,observableIdx:null}};var dsName=[];if(!isUndefinedOrNull(this.options.datasources))for(var i=0;i<this.options.datasources.length;i++)dsName.push(this.options.datasources[i].name);this.options.datasources.length>0&&(this.properties.frame.datasourceIdx=0);var dsListBoxId=OSH.Helper.HtmlHelper.addHTMLObjectWithLabelListBox(this.contentElt,"Data Source",this.options.datasources);this.observableListBoxId=OSH.Helper.HtmlHelper.addHTMLListBox(this.contentElt,"Observable",[]),this.loadObservable(dsListBoxId,this.observableListBoxId)||(this.properties.frame.observableIdx=0);var self=this;isUndefinedOrNull(properties)||OSH.Helper.HtmlHelper.onDomReady(function(){for(var dsTag=document.getElementById(dsListBoxId),idx=-1,i=0;i<dsTag.options.length;i++)if(dsTag.options[i].object.id===properties.datasourceId){idx=i;break}idx>-1&&(dsTag.options.selectedIndex=idx,document.getElementById(self.observableListBoxId).options.selectedIndex=properties.observableIdx)})},getProperties:function(){var stylerProperties={properties:{ui:{video:{}}}},dsIdsArray=[];for(var key in this.options.datasources)dsIdsArray.push(this.options.datasources[key].id);stylerProperties.properties.ui.video={},OSH.Asserts.checkObjectPropertyPath(this.properties,"frame"),OSH.Asserts.checkIsDefineOrNotNull(this.properties.frame.observableIdx),OSH.Asserts.checkIsDefineOrNotNull(this.properties.frame.datasourceIdx);var currentDatasource=this.options.datasources[this.properties.frame.datasourceIdx],observableIdx=document.getElementById(this.observableListBoxId).options.selectedIndex,videoFuncProps=OSH.UI.Styler.Factory.getVideoFunc(currentDatasource,observableIdx);return OSH.Utils.copyProperties(videoFuncProps,stylerProperties.properties),stylerProperties.properties.ui.video.observableIdx=observableIdx,stylerProperties.properties.ui.video.datasourceId=currentDatasource.id,stylerProperties}});
