...
OSH.UI.View.FFMPEGView = OSH.UI.View.VideoView.extend({
    ...
    initFFMPEG_DECODER_WORKER: function (callback) {
        this.worker = new Worker('js/workers-dev/osh-UI-FFMPEGViewWorker.js');
        ...
        this.worker.onmessage = function (e) {
            if(e.data !== null) {
                this.outputBuffer = e.data.outputBuffer;
                this.displayFrame(e.data.width,e.data.height,this.outputBuffer);

                self.worker.postMessage({
                    outputBuffer: this.outputBuffer,
                    release:true
                }, [
                    this.outputBuffer.y.buffer,
                    this.outputBuffer.u.buffer,
                    this.outputBuffer.v.buffer
                ]);
            }
        }.bind(this);
    ...
    displayFrame:function(width,height,frame) {
        if (!this.resetCalled) {
